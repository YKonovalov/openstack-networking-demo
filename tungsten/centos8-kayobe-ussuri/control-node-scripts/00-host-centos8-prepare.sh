...
function init_ssh(){
for h in ${N//,/ }; do
  ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null centos@$h date
  ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null centos@$h 'sudo sed -i "s/disable_root: 1/disable_root: 0/" /etc/cloud/cloud.cfg'
  cat /root/.ssh/authorized_keys|ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null centos@$h 'sudo tee /root/.ssh/authorized_keys'
  ssh -o StrictHostKeyChecking=no $h date
done
}
