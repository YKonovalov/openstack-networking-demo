...
variable "demospec" {
 type    = string
 default = ",os=wallaby,tf=latest,virt=kvm"
}
variable "userdata" {
 type    = string
 default = "../templates/centos8.yaml"
}
...
locals {
  ...
  hosts = templatefile("../templates/hosts", {
    hosts = join("\n", concat(
                 ...
                 [for o in local.hosts_vms : "${o.network[0].ip} ${o.name} ${split(".", o.name)[0]}"]
                 ))
    })
  genders = "${join("\n", concat(
...
resource "null_resource" "cluster" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "/bin/sh control-node-scripts/cluster-setup.sh",
    ]
  }
}
